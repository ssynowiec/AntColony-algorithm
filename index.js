...
class AntColony {
  constructor(cities, numAnts, alpha, beta, rho, q, startIndex) {
    this.cities = cities;
  ...
  selectNextCity(currentIndex, visited) {
...
const getCityNamesFromTour = (tour, cities) => {
  return tour.map((index) => cities[index].name).join(" -> ");
};

const getInputFromConsole = async (question) => {
...
const getIndexFromCityName = (cities, cityName) => {
